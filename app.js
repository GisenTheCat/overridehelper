...
async function checkForUpdates() {
    for (let i = 0; i < Object.keys(installedOverrides).length; i++) {
        ...
        if (overrides[overrideName] != undefined) {
            await fetch(overrides[overrideName].src)
                .then((res) => res.text())
                .then(async (text) => {
...
async function install(extension, button) {
    ...
    if (!overrides[extension].content) {
        overrides[extension].content = await fetch(overrides[extension].src)
            .then((res) => res.text())
            .then(
                (text) =>
                    `(function(){var originalScript=document.createElement('script');originalScript.src='${overrides[extension].url}?';document.head.appendChild(originalScript);})();var unsafeWindow=window;${text}`
            );
    }
...
function addButton(type, overridePath) {
    ...
    updateButton.innerText =
        type == 2 ? "UNINSTALL" : type == 0 ? "INSTALL" : "UPDATE";
    if (type == 2) {
...
async function start() {
    ...
    overrides = await fetch(baseUrl + "overrides.json").then((res) =>
        res.json()
    );
